...
def read_data(begin, f):
	...
	last = 65536 - begin
	try:
		code.fromfile(f, last)
	except EOFError:
		pass
	last = len(code)
	code.extend([0] * (65536 - last))
	return last - 1
...
def config(a):
	...
	parser = argparse.ArgumentParser(description="Dissassemble Z80 code")
	parser.add_argument("-c",'--comments', help="Show addresses of instructions after semicolon",
	required = False, action="store_true")
	parser.add_argument("-b",'--binary', help="Addresses of binary data. Ranges separated by comma. For example 1000-2000,3000-4000",
	required = False, default="",action="store")
	parser.add_argument("-i",'--input', help="Input filename. Default stdin", required = False, default="sys.stdin",action="store")
	parser.add_argument("-o",'--output', help="Output filename. Default stdout", required = False, default="sys.stdout",action="store")
	parser.add_argument("-s", '--start', help="Start address. Default 32768", required = False, default="32768", action="store")
	config = vars(parser.parse_args(a))
	main(config)
